...
const checkAI = () => {
  if (!client) {
    console.warn('MegaLLM API key not configured. Using fallback responses.');
    return false;
  }
  return true;
};
...
export const generateACTSPrompts = async (step: ACTSStep): Promise<string[]> => {
  ...
  try {
    ...
    if (!content) return ["Reflect on God's goodness.", "Consider His holiness.", "Rest in His presence."];

    const parsed = JSON.parse(content);
    return parsed.prompts || parsed.data || Object.values(parsed)[0] || [];
  } catch (error) {
...
export const weaveScripture = async (journalEntry: string): Promise<ScriptureWeaveResult | null> => {
  ...
  try {
    const prompt = `
      ...
      Return JSON with keys: verseText, reference, prayer
    `;
    ...
    return JSON.parse(content) as ScriptureWeaveResult;
...
export const generateIntercessionPrayer = async (name: string, request: string): Promise<string> => {
  ...
  try {
    ...
    return response.choices[0]?.message?.content || `Lord, we lift up ${name} to you. Your will be done in their life.`;
  } catch (error) {
...
export const synthesizePrayerSession = async (inputs: Record<string, string>): Promise<string> => {
  ...
  try {
    ...
    return response.choices[0]?.message?.content || "Lord, hear my prayer.";
  } catch (error) {
...
export const analyzeReflection = async (transcript: string): Promise<SpiritualPrescription | null> => {
  ...
  try {
    const prompt = `
      ...
      4. Write an "Insight" (Micro-Sermon): A 2-3 sentence deep theological encouragement or challenge based on the scripture.

      Return JSON with keys: emotion, diagnosis, verses (array with text and reference), insight
    `;
    ...
    const content = response.choices[0]?.message?.content;
    if (!content) return null;
    return JSON.parse(content) as SpiritualPrescription;
...
export const generateVisionBoard = async (focus: string): Promise<VisionCard | null> => {
  ...
  try {
    const prompt = `
      ...
      Return JSON with keys: visualKeyword, affirmation, scripture, reference
    `;
...
export const analyzePurposeAlignment = async (callings: string[], tasks: string[]): Promise<AlignmentResult | null> => {
  ...
  try {
    const prompt = `
      ...
      Return JSON with keys: score, feedback, analysis (array with task, status, reason)
    `;
...
export const generatePivotStrategy = async (habit: string, trigger: string): Promise<PivotStrategy | null> => {
  ...
  try {
    const prompt = `
      ...
      1. Create an "Interrupt Question": A jarring, thought-provoking question to immediately stop the autopilot behavior (e.g., "Is this what you want your son to remember?", "Does this feed the spirit or the flesh?").
      ...
      Return JSON with keys: interruptQuestion, scriptureTruth (object with text and reference), microAction
    `;
...
export const reframeConflict = async (rawThought: string): Promise<PeaceResponse | null> => {
  ...
  try {
    const prompt = `
      ...
      Return JSON with keys: originalEmotion, validation, reframedScript, biblicalPrinciple
    `;
    ...
    const content = response.choices[0]?.message?.content;
    if (!content) return null;
    return JSON.parse(content) as PeaceResponse;
